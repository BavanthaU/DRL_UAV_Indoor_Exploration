...
class Sb3VecEnvWrapper(VecEnv):
    ...
    def __init__(
        ...
        if self.discrete_action_space:
            action_space = gym.spaces.Discrete(num_action_vals)
        else:
            action_space = self.unwrapped.single_action_space
            if isinstance(action_space, gym.spaces.Box) and not action_space.is_bounded("both"):
                action_space = gym.spaces.Box(low=lower_bound, high=upper_bound, shape=action_space.shape)
    ...
    def get_occ_cells(self) -> list[float]:
        """Return the number of occupied cells of all the episodes"""
        return self._occ_cells_buf.cpu().tolist()
    ...
    def step_wait(self) -> VecEnvStepReturn:  # noqa: D102
        ...
        if self.discrete_action_space:
            obs_dict, rew, terminated, truncated, extras = self.env.step(self._async_actions.reshape(self.num_envs, 1))
        else:
        ...
        self._ep_len_buf += 1
        env_map = self.unwrapped.env_map
        ...
        self._occ_cells_buf[reset_ids] = 0
