...
class RLDiscreteSetpointController():
    ...
    def update(self, dt: float, action: torch.Tensor) -> torch.Tensor:
        ...
        Args:
            ...
            action (torch.Tensor): Integer between 0 to 5 indicating which linear and angular velocity commands apply in the body frame to move the drone in 2D (at a fixed height). Shape is (num_env).
    ...
    def update_ref(self, dt: float, action: torch.Tensor):
        """Method that updates the references for the controller to track. The action from the RL algorithm lives in a 1D discrete action space, with 6 possible values (0-5). 
        ...
        for i in range(self.num_envs):
            if action[i] == 0:
                cmd = [self.lin_vel, 0, 0]      # Go forward
            elif action[i] == 1:
                cmd = [-self.lin_vel, 0, 0]     # Go backwards
            elif action[i] == 2:
                cmd = [0, self.lin_vel, 0]      # Go right
            elif action[i] == 3:
                cmd = [0, - self.lin_vel, 0]    # Go left
            elif action[i] == 4:
                cmd = [0, 0, self.ang_vel]      # Turn right
            elif action[i] == 5:
                cmd = [0, 0, -self.ang_vel]     # Turn left
            else:
                raise ValueError('unknown action type: [{0:d}]'.format(action))

            vxy_world_frame =  self.R[i].as_matrix()  @ np.array([cmd[0], cmd[1], 0])
            self.v_ref[i, 0:2] = vxy_world_frame[0:2]
            self.yaw_rate_ref[i] = cmd[2]
