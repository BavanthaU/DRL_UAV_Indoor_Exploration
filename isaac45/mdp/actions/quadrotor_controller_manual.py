#!/usr/bin/env python
"""
| File: quadrotor_controller_manual.py
| Author: Andrea Bravo i Forn
   NOTE: 
   - Code adapted from Marcelo Jacinto and Joao Pinto (creators of Pegasus Simulator).
   - Computes body-wrench commands to be applied to the drone, in order to drive it with the keyboard in the 2D plane at a fixed height.
   - It needs to be used with the KeyBoardDrivenAction action term (set up from the class ActionsCfg with the KeyBoardDrivenActionCfg class).
   - The only drone-specific parameters needed are: drone mass (m), height at which the drone flies (self.z_pos), linear velocity modulus of the drone (self.lin_vel), and angular velocity modulus of the drone (self.ang_vel).
   - FLU drone frame assumed (x-axis = Forwards (red), y-axis = Left (green), z-axis = Up (blue)).
   - In the prim tree, the drone body must be the first item (otherwise the body_ids in the KeyBoardDrivenAction class needs to be changed).
...
# Import libraries for keybord input
import sys
import threading
import select
...
class KeyboardInputActions:
    """ Transforms keybord inputs to 2D velocity commands (setpoints for the non-linear controller). 
    """
    def __init__(self):
        ...
        self.action_mod = np.linalg.norm(np.array([1, 1, 0]))
        self.current_action = np.array([0, 0, 0])

    def listen_to_keyboard(self):

        instructions = """
        To drive a drone with the keyboard press the any of these keys + Enter:
        - A: Go left
        - D: Go right
        - X: Go backwards
        - W: Go forward
        ...
        - E: Go forward-right
        - Z: Go backwards-left
        - C: Go backwards-right
        - S: Stop
        - P: Turn right
        - O: Turn left

        Note: These instructions are in the drone-attached frame coordinates.
        """
        ...
        try:
            while self.running:
                if sys.stdin in select.select([sys.stdin], [], [], 0.1)[0]:
                    ...
                    if key in ['a', 'd', 'w', 'x', 'q', 'e', 'z', 'c', 's', 'p', 'o']:
    ...
    def get_action(self):
        # self.current_action = np.array([0, 0, 0]) # No movement if no key has been ever pressed
                
        if self.pressed_key == 'x':
            self.current_action = np.array([-1, 0, 0])

        if self.pressed_key == 'w':
            self.current_action = np.array([1, 0, 0])
  
        if self.pressed_key == 'd':
            self.current_action = np.array([0, -1, 0])
     
        if self.pressed_key == 'a':
            self.current_action = np.array([0, 1, 0])

        if self.pressed_key == 'q':
            self.current_action = np.array([1, 1, 0])/self.action_mod
        
        if self.pressed_key == 'e':
            self.current_action = np.array([1, -1, 0])/self.action_mod
        
        if self.pressed_key == 'z':
            self.current_action = np.array([-1, 1, 0])/self.action_mod
        ...
        if self.pressed_key == 'p':
            self.current_action = np.array([0, 0, -1])
   
        if self.pressed_key == 'o':
            self.current_action = np.array([0, 0, 1])
        return self.current_action


class KeyboardSetpointController():
    """ It implements a nonlinear controller that allows a drone to track aggressive trajectories. 
    """
    ...
    def __init__(self,  
        ...
        # Define manual driver 
        self.Driver =  KeyboardInputActions()
    ...
    def update_state(self, state: torch.Tensor):
        ...
        NOTE:
            - self.p: numpy array of size (num_envs, 3) with the positions [x,y,z] of the drone-attached frame in each environment, w.r.t the world inertial frame, expressed in the world frame.
            - orientation_quat: numpy array of size (num_envs, 4) with the quaternions [qx, qy, qz, qw] encoding the orientation of the drone-attached frame in each environment, w.r.t the world frame, expressed in the world frame.
                                The input quaterions have to be re-ordered to fit the scipy.spatial.transform library convention.
            - self.R: Rotation element storing the rotation matrices of the drone-attached frame in each environment, w.r.t the world frame, expressed in the world frame.
            - self.w: numpy array of size (num_envs, 3) with the [w_x,w_y,w_z] angular velocities of the drone-attached frame in each environment, w.r.t the world frame, expressed in the drone FLU frame.
            - self.v: numpy array of size (num_envs, 3) with the [v_x,v_y,v_z] linear velocities of the drone-attached frame in each environment, w.r.t the world frame, expressed in the world frame.
    ...
    def update(self, dt: float) -> torch.Tensor:
        """Method that updates the target wrench applied to the drone body, based on keyboard inputs and executing a nonlinear control law.  
        This method will be called by the simulation on every physics step.

        Args:
            dt (float): The time elapsed between the previous and current function calls (s).
        """
        action =  self.Driver.get_action() 
    ...
    def update_ref(self, dt: float, action: np.ndarray):
        """Method that updates the reference signal tracked by the controller based on the setpoint recieved from the keyboard.
        ...
        Args:
            dt (float): The time elapsed between the previous and current function calls (s).
            action (np.ndarray): linear and angular velocity commands in the body frame to move the drone in 2D (at a fixed height). Shape is (3) with: [v_x, v_y, w_z] -> v_x > 0 (go forward), v_x < 0 (go backwards) / v_y > 0 (go left), v_y < 0 (go right) / w_z > 0 (turn CCW), w_z < 0 (turn CW)
                                 If we have multiple environments, in all the environmnets the drone will move in the same way.
        """

        for i in range(self.num_envs):
            vxy_world_frame =  self.R[i].as_matrix()  @ np.array([action[0], action[1], 0])
            self.v_ref[i, 0:2] = vxy_world_frame[0:2] * self.lin_vel
            self.yaw_rate_ref[i] = action[2] * self.ang_vel
