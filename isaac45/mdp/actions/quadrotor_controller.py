...
class RLSetpointController():
    ...
    def update(self, dt: float, action: torch.Tensor) -> torch.Tensor:
        ...
        Args:
            ...
            action (torch.Tensor): linear and angular velocity commands in the body frame to move the drone in 2D (at a fixed height). Shape is (num_env, 3). In the 2nd dim: [v_x, v_y, w_z] -> v_x > 0 (go forward), v_x < 0 (go backwards) / v_y > 0 (go left), v_y < 0 (go right) / w_z > 0 (turn CCW), w_z < 0 (turn CW)
    ...
    def update_ref(self, dt: float, action: torch.Tensor):
        """Method that updates the references for the controller to track. The action from the RL algorithm lives in a 3D, bouned, box-like continuous action space. 
        ...
        for i in range(self.num_envs):
            vxy_world_frame =  self.R[i].as_matrix()  @ np.array([action[i, 0], action[i, 1], 0])
            self.v_ref[i, 0:2] = vxy_world_frame[0:2] * self.max_lin_vel
            self.yaw_rate_ref[i] = action[i,2] * self.max_ang_vel
