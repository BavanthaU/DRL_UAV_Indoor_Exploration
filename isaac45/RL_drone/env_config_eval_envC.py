...
class QuadrotorSceneCfg(InteractiveSceneCfg):
    # Testing Office environment C
    office_A = AssetBaseCfg(
        prim_path = "{ENV_REGEX_NS}/officeC", 
        spawn=sim_utils.UsdFileCfg(usd_path=str(Path("DRL_UAV_Indoor_Exploration/isaac45/environments/TestEnvOfficeC.usd").resolve())), 
...
class EventCfg:
    ...
    # reset event
    reset_drone_eval_envC = EventTerm(
        func=mdp.events.reset_drone_eval_envC,  # Using reset_joints_by_offset function
        mode="reset",
    )
...
class RewardsCfg:
    ...
    exploration_finished = RewTerm(func=mdp.rewards.fixed_area_covered, weight=150.0, params={"num_cells_to_cover":7000})
...
class TerminationCfg:
    ...
    area_is_covered=DoneTerm(func=mdp.terminations.drone_covers_fixed_area, params={"num_cells_to_cover":7000},)
