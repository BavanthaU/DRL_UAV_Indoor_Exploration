...
class QuadrotorSceneCfg(InteractiveSceneCfg):
    # Testing Office environment A
    office_A = AssetBaseCfg(
        prim_path = "{ENV_REGEX_NS}/officeA", 
        spawn=sim_utils.UsdFileCfg(usd_path = str(Path("DRL_UAV_Indoor_Exploration/isaac45/environments/TestEnvOfficeA.usd").resolve())), 
...
@configclass
class ActionsCfg:   
    # drone_drive_vel_setpoint = RLDriven2DActionCfg(asset_name="robot")
...
class EventCfg:
    ...
    # reset event
    reset_drone_eval_envA = EventTerm(
        func=mdp.events.reset_drone_eval_envA,  # Using reset_joints_by_offset function
        mode="reset",
    )
...
class RewardsCfg:
    ...
    exploration_finished = RewTerm(func=mdp.rewards.fixed_area_covered, weight=150.0, params={"num_cells_to_cover":4350})
...
class TerminationCfg:
    ...
    area_is_covered=DoneTerm(func=mdp.terminations.drone_covers_fixed_area, params={"num_cells_to_cover":4350},)
...
class DroneEnvCfg(ManagerBasedRLEnvCfg):
    ...
    def __post_init__(self) -> None:
        ...
        # viewer settings (top-down view to inspect coverage)
