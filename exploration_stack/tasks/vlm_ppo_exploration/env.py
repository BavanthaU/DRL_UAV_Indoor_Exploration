...
from .terminations import altitude_out_of_bounds, coverage_success
...
class IsaacVlmPpoUavExplorationEnv(DirectRLEnv):
    ...
    def __init__(self, cfg: IsaacVlmPpoUavExplorationEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        self._prev_coverage_ratio = torch.zeros_like(self._coverage_ratio)
        ...
        self._reward_weights = RewardWeights(
            success_threshold=self.cfg.map_cfg.success_threshold,
            w_success=self.cfg.reward_cfg.w_success,
            w_global_coverage_progress=self.cfg.reward_cfg.w_global_coverage_progress,
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        extrinsic, extrinsic_terms = compute_extrinsic_reward(
            ...
            prev_coverage_ratio=self._prev_coverage_ratio,
            collision=collision,
        ...
        self._prev_coverage_ratio[:] = self._coverage_ratio
        all_terms = {**extrinsic_terms, **intrinsic_terms, "new_cells": new_counts, "coverage": self._coverage_ratio}
    ...
    def _get_dones(self) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        success = coverage_success(self._coverage_ratio, self.cfg.map_cfg.success_threshold)
        stuck = self._stuck_counter >= self.cfg.map_cfg.stuck_steps
        return altitude_done | success | stuck, time_out
    ...
    def _reset_idx(self, env_ids: torch.Tensor | None):
        ...
        self._coverage_ratio[env_ids] = 0.0
        self._prev_coverage_ratio[env_ids] = 0.0
    ...
    def _update_coverage_memory(self):
        centers = self._world_to_grid(self._robot.data.root_pos_w[:, :2])
        ...
        self._coverage_ratio = new_counts.float() / float(self.cfg.map_cfg.grid_size * self.cfg.map_cfg.grid_size)
