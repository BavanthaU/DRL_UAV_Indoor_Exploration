...
def main() -> None:
    parser = argparse.ArgumentParser(description="Label hierarchical rollouts with offline Qwen2.5-VL.")
    parser.add_argument("--rollout", required=True, help="Path to hierarchical_rollout.pt")
    ...
    parser.add_argument("--wandb_project", default="vlm-hierarchical-ppo-uav-exploration")
    ...
    frame_limit = args.max_frames if args.max_frames is not None else len(frames)
    ...
    with output.open("w", encoding="utf-8") as file:
        for frame_id, frame in enumerate(frames[:frame_limit]):
            ...
            file.write(
                json.dumps(
                    {
                        "frame_id": frame_id,
                        "option": frame.get("option", torch.empty(0)).tolist(),
                        "candidate": frame.get("candidate", torch.empty(0)).tolist(),
    ...
    if args.wandb_mode != "disabled":
        ...
        run.finish()
    print(f"[INFO] Saved hierarchical Qwen affordance labels: {output}")
